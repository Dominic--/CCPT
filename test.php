...
function http_curl($url, $postfields) {
	if (close_curl) {
		if (file_exists(dirname(__FILE__).'/wp-config.php')) {
			...
			return http_curl_wp($url, $postfields);
		}
	}
	$ch = curl_init();
...
function http_curl_wp($url, $postfields) {
	if ($postfields) {
...
function function_support(&$func_items) {
	...
	foreach($func_items as $item) {
		...
		if ($item == "close_curl") {
			...
			//$func_str .= "<td>Zend Guard Loader ";
			$func_str .= $version;
			if (!$status) {
				$func_str .= " <span style=\"color:green\">不支持Zend Guard Loader，意味着不能使用相关主题。 php5.2请升级到php5.3或php5.4, php5.3及以上版本请安装Zend Guard Loader</span>";
			} elseif (version_compare(PHP_VERSION, '5.5', '>=')) {
				$func_str .= '<span style=\"color:red\">很遗憾，暂时不能在php5.5上使用相关主题。请降到PHP5.4或者PHP5.3</span>';
			} /*else {
				$func_str .= ( version_compare($version, '3.3', '<') ) ? " <span style=\"color:red\">版本太低，php5.2.x请升级到3.3.0或以上版本，否则不能使用 相关主题</span>" : '';
			}*/
...
if (version_compare(PHP_VERSION, '5.5', '>=')) {
	$zend_install_tips = '很遗憾，您正在使用PHP 5.5以上版本，暂时不能使用相关主题。请降到PHP5.4或者PHP5.3';
} else {
	$zend_loader_enabled = function_exists('zend_loader_enabled');
	if (($zend_loader_enabled)&&(version_compare(PHP_VERSION, '5.3', '>='))) {//安装zend组件而且php大于等于5.3
		...
		if (version_compare($zend_loader_version, '3.3', '>=')) {
			...
			$zend_install_tips = '恭喜你，您的服务器支持安装相关主题，您的php版本是 ' . PHP_VERSION .'。<a href="http://devework.com/theme/" target="_blank">现在去购买</a>';
		} else {
			$zend_install_tips = '很遗憾，您 不能 使用相关主题，您的php版本是 ' . PHP_VERSION .'，但zend版本太低，请升级到3.3.0或以上版本，<a href="http://www.zend.com/en/products/guard/downloads" target="_blank">查看</a>';
			$zend_install_tips .= '【<a href="http://pan.baidu.com/s/1ntwUdlB" target="_blank">网盘下载Zend</a>】';
		} 
	} else {//没有安装zend
		if (version_compare(PHP_VERSION, '5.3', '>=')) {//没有安装zend --大于等于5.3
			$zend_install_tips = '很遗憾，您 不能 使用相关主题，您的php版本是 ' . PHP_VERSION .'，php5.3或php5.4版本请安装 <a href="http://www.zend.com/en/products/guard/downloads" target="_blank">Zend Guard Loader</a>';
			$zend_install_tips .= '【<a href="http://pan.baidu.com/s/1ntwUdlB" target="_blank">网盘下载Zend</a>】';
		} else {//没有安装zend --低于5.3
			$zend_install_tips = '很遗憾，您 不能 使用相关主题，您的php版本是 ' . PHP_VERSION .'，请升级你的php版本到PHP5.4或者PHP5.3';
		} 
